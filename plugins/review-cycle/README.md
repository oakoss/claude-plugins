...
Scripts and shell functions are opaque: `./release.sh` runs whatever it contains. So after any Bash call that moves HEAD, the gate checks the new commit too. A commit that slipped past the check — a script that commits, or a pre-commit hook that rewrote a file after the check — is reported back to the agent, which is told to tell you. A HEAD that moves to an existing commit is not a commit and is not reported: a checkout, a reset, a fast-forward, `gh pr merge` pulling the merged branch. A commit exists already when a ref reached it before the command ran or a fetch brought it in, through a branch or remote-tracking ref the fetch moved, or through what `FETCH_HEAD` names when the command pushed nothing. A push is seen by the remote-tracking ref it moves or creates, which git logs as `update by push`: one the gate did not check is reported when your latest message did not ask for a push. Not seen: a push to a URL or a remote with no tracking ref, one that deletes a remote branch, one undone before the command ends, one whose reflog git did not keep, and one buried under more than 50 later moves of the same ref in one command. When such a push is followed by a fetch that brings its commit back, that commit is not reported either.
