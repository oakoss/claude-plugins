...
If either differs, name the leg and report it in the summary. The status tool's `reviewerChanges` names each Claude reviewer command the gate saw change the repository, and what it changed. It lasts the whole session, so record how many entries it had with the Phase 3 snapshot and read only the ones added since. The gate never compares around the Codex leg, since it runs as the main session's background command, nor after a reviewer's background command returns. So a difference with no new entry most likely came from Codex, or from a write the gate does not check. The commit gate already refuses the content a leg changed, since no review covers an edit made while it ran; the report tells the user why.
...
**Every brief carries what earlier cycles settled.** Before composing the first brief, call the `mcp__review-cycle__ledger` tool with the changed-file list as `paths`. It returns the findings earlier review cycles in this repository deferred, rebutted, left alone on purpose, or raised as questions, each with an id, `changed` when its file differs from what the last cycle to carry it reviewed, and `stale` when it was settled more than 90 days ago. Append them as a `settled in earlier cycles` block on the same terms as the block below, with these exceptions, each listed as reopened so the legs judge it afresh: a `changed` entry whose `git diff <blob> <current>` touches the code it cites, or cannot be run; an entry whose `changed` is null, since nothing checked it; and a `stale` one. Record the entries' ids, and which were reopened, in the summary draft, with any `note` the tool reported; Phase 9 needs them. Stored entries it counts as `unreadable` are dropped by the next record, which reports them. When the tool does not exist, or answers with an error rather than JSON, write `Ledger: unavailable (<its answer, or tool missing>)` into the draft and continue.
...
**Record the cycle in the ledger first**, with the `mcp__review-cycle__ledger_record` tool, so the next cycle's reviewers do not re-raise what this one settled. Pass as `entries` every finding this cycle deferred, rebutted, or examined and left alone on purpose, and every question, each with its path, line when it has one, kind (`deferred`, `rebutted`, `left-alone`, `question`), the finding, the reason (the deferral criterion, the measurement or documentation that rebutted it, or why it was left alone), and the leg that raised it. Pass as `resolve` the ids of carried entries this cycle fixed, or found no longer apply. Pass as `keep` the ids of every other carried entry that was not reopened: the tool stamps each with its file as this cycle reviewed it, keeps its date, and drops one whose file is gone. A reopened entry the legs settled again goes in `entries` as this cycle now sees it, which dates it today, with its old id in `resolve` when the wording changed. A fixed finding is never recorded: the code changed, so raising it again means it came back. When there is nothing to record, keep or resolve, skip the call. The tool refuses a malformed batch whole and names the first fault; correct it and call again. When the tool does not exist or cannot record, the summary says `not recorded (<its answer, or tool missing>)`. Skip this step when the cycle was aborted.
...
Ledger: carried N from earlier cycles (O reopened); recorded A new, U updated, K kept, R resolved[, G gone][, E evicted][, D dropped unreadable][; <note>] | unavailable (<reason>) | not recorded (<reason>)
