...
@test "marketplace drift fails --check and names the plugin" {
  ...
  run_script --check
  [ "$status" -eq 1 ]
  [[ "$output" == *"alpha: marketplace.json 0.9.0"* ]]
}
...
@test "package.json vs plugin.json drift fails --check" {
  ...
  run_script --check
  [ "$status" -eq 1 ]
  [[ "$output" == *"plugin.json 1.0.0 != package.json 1.1.0"* ]]
}
...
@test "missing changelog heading for the version fails --check" {
  ...
  run_script --check
  [ "$status" -eq 1 ]
  [[ "$output" == *"CHANGELOG.md has no heading for 1.0.0"* ]]
}
...
@test "plugin without package.json is checked for manifest agreement only" {
  ...
  run_script --check
  [ "$status" -eq 1 ]
  [[ "$output" == *"alpha: marketplace.json 1.0.0 != 0.9.0"* ]]
}
...
@test "plugin missing from marketplace.json is reported" {
  ...
  run_script --check
  [ "$status" -eq 1 ]
  [[ "$output" == *"beta: no entry in marketplace.json"* ]]
}
